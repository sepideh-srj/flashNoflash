...
class CyclePix2PixLabModel(BaseModel):
    ...
    def forward(self):
        ...
        if self.opt.midas:
            decomposition_input = torch.cat((self.real_A, self.midas_A), 1)
            generation_input = torch.cat((self.real_B, self.midas_B), 1)
        else:
            decomposition_input = self.real_A
            generation_input = self.real_B
        ...
        if self.opt.ratio:
            ...
            self.fake_A = self.applyratio(self.real_B,generation_output)
        else:
            self.fake_B = decomposition_output
            self.fake_A = generation_output
        ...
        ## cycle Adding depth if needed
        if self.opt.midas:
            decomposition_input_cycle = torch.cat((self.fake_A, self.midas_B), 1)
            generation_input_cycle = torch.cat((self.fake_B, self.midas_A), 1)
        else:
            decomposition_input_cycle = self.fake_A
            generation_input_cycle = self.fake_B

        ## cycle forward into networks
        decomposition_output_cycle = self.netG_Decompostion(decomposition_input_cycle)
        generation_output_cycle = self.netG_Generation(generation_input_cycle)

        ## cycle applying ratio if needed
        if self.opt.ratio:
            self.rec_B = self.applyratio(self.fake_A,decomposition_output_cycle)
            self.rec_A = self.applyratio(self.fake_B,generation_output_cycle)
        else:
            self.rec_B = decomposition_output_cycle
            self.rec_A = generation_output_cycle
