...
class AlignedLabDataset(BaseDataset):
    ...
    def __init__(self, opt):
        ...
        self.AB_paths = sorted(make_dataset(self.dir_AB, opt.max_dataset_size))  # load images from '/path/to/data/trainB'
    ...
    def __getitem__(self, index):
        ...
        if 'flash' in path:
            ...
            ambient_pic_adjust = Image.fromarray((ambient_pic_adjust * 255).astype('uint8'))
            ...
            ambient_out = alpha_blend(ambient_pic, ambient)

            flash_out = (flash_out * 255).astype('uint8')
            flash_out = Image.fromarray(flash_out)
            ambient_out = (ambient_out * 255).astype('uint8')
            ambient_out = Image.fromarray(ambient_out)
...
def gama_corect(rgb):
    ...
    srgb = np.zeros_like(rgb).astype(np.float)
    rgb = rgb
    # print(rgb)
    mask1 = (rgb>0)*(rgb<0.0031308)
    mask2 = (1-mask1).astype(bool)
