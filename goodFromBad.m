...
destinationFolder = 'bad/goodFromBad/good';
if ~exist(destinationFolder, 'dir')
  mkdir(destinationFolder);
end
destinationFolder2 = 'bad/goodFromBad/bad';
...
%     
    [crit,x] = getBrightness(flash2, illuminantCode(des))
    flash2 = flash2 * x;
    [crit,x2] = getBrightness(flash2, illuminantCode(des))
    if (crit > 1500)
        dataGood(end+1,:) = [i, crit];
        fullDestinationFileName2 = fullfile(destinationFolder, im);
        imwrite (flash*x,fullDestinationFileName2,'Comment',rawFile.Comment,'Description', rawFile.Description)
...
%     input = xyz2rgb(flash2, 'WhitePoint', illuminantCode(des));
%     output = xyz2rgb(ambient2, 'WhitePoint', illuminantCode(des));
%     figure;
%     imshow(input)
% %     output = real(output);
%     
